...
let pp_span_custom (fmt : Format.formatter) (span : Trace.span) =
  ...
  in
  Format.fprintf fmt
    "{ trace_id = %s; span_id = %s; parent_span_id = %s; name = %s; kind = %a; start_time_unix_nano = %Ld; end_time_unix_nano = %Ld; attributes = %a; dropped_attributes_count = %ld; events = %a; dropped_events_count = %ld; links = %a; dropped_links_count = %ld; status = %a }"
