...
let rec to_string (node : span_tree_node) : string =
  let rec children_to_string l acc =
    match l with
    | [] -> acc ^ " ]"
    | h :: [] -> children_to_string [] (acc ^ to_string h)
    | h :: t -> children_to_string t (acc ^ to_string h ^ ", ")
  in
  Printf.sprintf "{ \"span\": %s, \"children\": %s }"
    (Otel_encoder.to_json_string node.span)
    (children_to_string node.children "[")

let list_to_json_string (l : span_tree_node list) : string =
  let rec map_to_string_aux (l : span_tree_node list) acc =
    match l with
    | [] -> acc ^ " ]"
    | h :: [] -> map_to_string_aux [] (acc ^ to_string h)
    | h :: t -> map_to_string_aux t (acc ^ to_string h ^ ", ")
  in
  map_to_string_aux l "[ "
