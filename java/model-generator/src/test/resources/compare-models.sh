...
if [[ ! -f "$FILE1" || ! -f "$FILE2" ]]; then
  echo "Usage: $0 file1.json file2.json threshold"
  exit 1
fi
...
printf "%-10s            | %-10s           | %-10s | %-10s | %-25s\n" "Prob1" "Prob2" "Nr1" "Nr2" "DeclareTemplate"
printf "%s\n" "----------------------+----------------------+------------+------------+--------------------------------------------------"
...
while read -r template; do
  ...
  # Calculate absolute difference using awk (avoiding bc completely)
  abs_diff=$(awk "BEGIN {diff = $prob1 - $prob2; print (diff < 0) ? -diff : diff}")

  # Check if different and above threshold using awk
  above_threshold=$(awk "BEGIN {print ($abs_diff > $THRESHOLD) ? 1 : 0}")

  if [[ "$nr1" != "$nr2" || "$above_threshold" -eq 1 ]]; then
    ((changed_count++))
    printf "%-10s | %-10s | %-10s | %-10s | %-25s \n" "$prob1" "$prob2" "$nr1" "$nr2" "$template"
  else
    ((unchanged_count++))
  fi
done < "$COMMON"
